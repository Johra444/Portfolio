        ...
        <header className='flex z-50 w-full h-14 bg-[#F4EDE4] dark:bg-gris sticky top-0'>
            <div class="container max-w-screen-md mx-auto px-5 flex items-center relative">
                ...
                <div className='ml-auto'>
                    {currentTheme === 'dark' ? (
                        <button className=' h-10 w-10 bg-peach opacity-1 rounded-lg' onClick={() => setTheme('light')}><SunIcon color={'black'} boxSize={16} /></button>
                        ) : (
                        <button className=' h-10 w-10 bg-myPurple opacity-1 rounded-lg' onClick={() => setTheme('dark')}><MoonIcon color={'white'} boxSize={16} /></button>   
                    )}
                </div>
            </div>
        </header>
